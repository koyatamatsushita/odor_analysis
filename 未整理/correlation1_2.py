...
def main():
    ...
    # 相関係数を計算
    # 計算用のデータフレームを作る
    cor1_df = pd.concat([transformed1.iloc[:, 0], stan_data_df], axis=1, sort=False)
    cor2_df = pd.concat([transformed1.iloc[:, 1], stan_data_df], axis=1, sort=False)
    # 相関データフレーム
    cor1_df = cor1_df.corr()["PC1"]
    cor2_df = cor2_df.corr()["PC2"]
    
    # pandasとmatplotlibで描画-------------------------------------------------------------
    # 相関係数降順
    # カラーインデックスの作成
    color_index=[]
    for i in range(len(cor1_df.drop("PC1"))):
        color_index.append(cm.jet(0.9-(i/6%1)))
    
    # 色指定用
    color_df = pd.DataFrame(color_index, index=COLUMNS48)
    sort_cor1_df = pd.concat([cor1_df.drop("PC1"), color_df], axis=1)
    sort_cor2_df = pd.concat([cor2_df.drop("PC2"), color_df], axis=1)
    sort_cor1_df = sort_cor1_df.sort_values("PC1", ascending=False)
    sort_cor2_df = sort_cor2_df.sort_values("PC2", ascending=False)
    # カラーインデックス並び替え版
    sort_color_index1 = []
    sort_color_index2 = []
    for i in range(len(sort_cor1_df)):
        sort_color_index1.append(tuple(sort_cor1_df.drop(["PC1"], axis=1).iloc[i]))
    for i in range(len(sort_cor2_df)):
        sort_color_index2.append(tuple(sort_cor2_df.drop(["PC2"], axis=1).iloc[i]))
    # 並べ替え版相関係数
    cor1_df = cor1_df.drop("PC1")
    cor2_df = cor2_df.drop("PC2")
    sort_cor1_df = cor1_df.sort_values(ascending=False)
    sort_cor2_df = cor2_df.sort_values(ascending=False)
    ...
    plt.subplot(2, 1, 1)
    # 相関の棒グラフのプロット
    sort_cor1_df.plot.bar(color=sort_color_index1, edgecolor="0", linewidth=2)
    plt.title("PC1")
    plt.xlabel("48index")
    ...
    plt.subplot(2, 1, 2)
    # 相関の棒グラフのプロット
    sort_cor2_df.plot.bar(color=sort_color_index2, edgecolor="0", linewidth=2)
    plt.title("PC2")
    plt.xlabel("48index")
    plt.ylabel("correlation")
    # グラフを表示する
    ...
    # 相関が0.45以上の指標を抜き出す
    new_stan_data_df = stan_data_df.drop(cor1_df[cor1_df<0.45].index, axis=1)
    new_stan_data = new_stan_data_df.values
    
    # 主成分分析してプロット
    pca_2Dplot(q_stan_data, new_stan_data, odor)
